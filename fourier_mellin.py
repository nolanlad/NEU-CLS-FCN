...
def mellin_fourier(im):
    imgData = addNoiseToImage(im, noiseMode, noiseIntensity)
    imgFft, imgFftShifted = calculateFft(imgData) # FFT of the image
    imgMags = cv2.magnitude(imgFftShifted[:,:,RE_IDX],imgFftShifted[:,:,IM_IDX])
    imgOriginal, imgOriginalFft, imgOriginalMags = (imgData, imgFftShifted, imgMags)
